...
df = pd.read_csv("../data/campaign_testmode_0720-0726.txt",header=None,sep=',',index_col=None,names=['id','idwebsite','date_added',
...
### Add purchase or not
df_transaction = pd.read_csv("../data/transaction07_withdate_complete.txt", index_col=None,header=None,sep='\t',names=['price','currency','idclick','date_added_full'])
...
df_default = pd.read_csv("default_price_currency.txt",header=0, index_col=0, sep=',')
...
df_countrylist = pd.read_csv("../data/compare_country_code/country_list.csv",header=None,index_col=None,names=['code_string','country_code'])
...
### Add vertical type
df_webVType = pd.read_csv("../data/websiteverticleidapp.csv",header=None,index_col=None,names=['idwebsite','idVtype','idApp'],sep='\t')
df_app = pd.read_csv("../data/apps.csv",header=None, index_col=None,names=['idApp', 'idcategory', 'type', 'end_date', 'change_date', 'date_added'],sep='\t')
...
#####Generate output
df_merge = df_merge[['hour','weekday','country_code','idoperator','iddevice','idhardware','idbrowser','idos','idcampaign','idcat','uniform_price','purchase','date_added_full']]
df_merge.fillna(0, inplace=True)

df_merge.to_csv("campaign_testmode_1w_index.txt")
